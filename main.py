...
if __name__ == "__main__":
    ...
    threading.Thread(target=server.start).start()  # runs server forever in background
    ...
    api_thread = threading.Thread(target=ca.start, daemon=True)
    api_thread.start()

    parse_path_for_boards(
        os.path.join(os.path.abspath(os.path.dirname(__file__)), "boards")
    )
    print(BOARDS)
    time.sleep(2)
