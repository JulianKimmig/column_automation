...
class RAMPS_14_AccelStepper(ArduinoBasicBoard):
    ...
    def __init__(self):
        super().__init__()
        self.save_attributes.update({"position":"double","steps_per_mm":"double","max_mm_sec":"double","acceleration":"double",})
    ...
    def addPortCommands(self):
        ...
        def receive_position_x(data):
            self.set_position([data/self._steps_per_mm[0],self._position[1],self._position[2]],to_board=False)
            if self.identified:
                self.serialport.data_to_socket(key=str(self.id) + "_position_x", y=data/self._steps_per_mm[0], x=None)
        ...
        def receive_position_y(data):
            self.set_position([self._position[0],data/self._steps_per_mm[1],self._position[2]],to_board=False)
            if self.identified:
                self.serialport.data_to_socket(key=str(self.id) + "_position_y", y=data/self._steps_per_mm[1], x=None)
        ...
        def receive_position_z(data):
            self.set_position([self._position[0],self._position[1],data/self._steps_per_mm[2]], to_board=False)
            if self.identified:
                self.serialport.data_to_socket(key=str(self.id) + "_position_z", y=data/self._steps_per_mm[2], x=None)
    ...
    def set_position(self, xyz, to_board=True):
        try:
            ...
            xyz=[0,0,0]
        steps=[int(self.steps_per_mm[0]*xyz[0]),int(self.steps_per_mm[1]*xyz[1]),int(self.steps_per_mm[2]*xyz[2])]

        if self._position[0] !=xyz[0]:
            self.serialport.logger.info("set x: " + str(xyz[0]))
            if to_board:
                self.get_portcommand_by_name("x_set_position").sendfunction(
                    steps[0]
                )
        if self._position[1] !=xyz[1]:
            self.serialport.logger.info("set y: "+str(xyz[1]))
            if to_board:
                self.get_portcommand_by_name("y_set_position").sendfunction(
                    steps[1]
                )
    ...
    def move_to(self, xyz, to_board=True):
        ...
        if self._position[0] !=xyz[0]:
            ...
            if to_board:
                self.get_portcommand_by_name("x_move_to").sendfunction(
                    int(steps[0])
                )
        if self._position[1] !=xyz[1]:
            self.serialport.logger.info("move to y: " + str(xyz[1]))
            if to_board:
                self.get_portcommand_by_name("y_move_to").sendfunction(
                    int(steps[1])
                )
        if self._position[2] !=xyz[2]:
            self.serialport.logger.info("move to z: " + str(xyz[2]))
            if to_board:
                self.get_portcommand_by_name("z_move_to").sendfunction(
                    int(steps[2])
                )
    ...
    def set_max_mm_sec(self, max_mm_sec, to_board=True):
        max_steps_sec = [self.steps_per_mm[0]*max_mm_sec[0],self.steps_per_mm[1]*max_mm_sec[1],self.steps_per_mm[2]*max_mm_sec[2]]
        if self._max_mm_sec[0] !=max_mm_sec[0]:
            self.serialport.logger.info("set x speed: " + str(max_mm_sec[0]))
            if to_board:
                self.get_portcommand_by_name("x_set_max_speed").sendfunction(max_steps_sec[0])
        if self._max_mm_sec[1] !=max_mm_sec[1]:
            self.serialport.logger.info("set y speed: " + str(max_mm_sec[1]))
            if to_board:
                self.get_portcommand_by_name("y_set_max_speed").sendfunction(max_steps_sec[1])
        if self._max_mm_sec[2] !=max_mm_sec[2]:
            self.serialport.logger.info("set z speed: " + str(max_mm_sec[2]))
            if to_board:
                self.get_portcommand_by_name("z_set_max_speed").sendfunction(max_steps_sec[2])
        self._max_mm_sec=max_mm_sec

    def set_acceleration(self, acceleration, to_board=True):
        steps_acceleration = [self.steps_per_mm[0]*acceleration[0],self.steps_per_mm[1]*acceleration[1],self.steps_per_mm[2]*acceleration[2]]
        if self._acceleration[0] !=acceleration[0]:
            self.serialport.logger.info("set x acceleration: " + str(acceleration[0]))
            if to_board:
                self.get_portcommand_by_name("x_set_acceleration").sendfunction(steps_acceleration[0])
        if self._acceleration[1] !=acceleration[1]:
            self.serialport.logger.info("set y acceleration: " + str(acceleration[1]))
            if to_board:
                self.get_portcommand_by_name("y_set_acceleration").sendfunction(steps_acceleration[1])
        if self._acceleration[2] !=acceleration[2]:
            self.serialport.logger.info("set z acceleration: " + str(acceleration[2]))
            if to_board:
                self.get_portcommand_by_name("z_set_acceleration").sendfunction(steps_acceleration[2])
